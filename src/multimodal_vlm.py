...
ALT_MODELS = ["qwen2.5-vl", "qwen3-vl", "ministral-3:14b", "llava:7b"]
...
def query_ollama_vlm(image_path, prompt, model=MODEL_NAME, temperature=0.7):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Error from Ollama model '{model}' ({response.status_code}): {response.text}")
            return None
    except Exception as e:
        print(f"Request failed for model '{model}': {e}")
        return None

def query_vlm_with_fallback(image_path, prompt, temperature=0.7):
    resp = query_ollama_vlm(image_path, prompt, model=MODEL_NAME, temperature=temperature)
    if resp:
        return resp
    for alt in ALT_MODELS:
        print(f"Fallback VLM query to recommended alternative model '{alt}'...")
        resp = query_ollama_vlm(image_path, prompt, model=alt, temperature=temperature)
        if resp:
            return resp
    return None

def run_task1():
    print("=== Running Multimodal LLM Description ===")
    dataset_dir = os.path.abspath('assignment3_dataset/nuclei_dataset')
    metadata_path = os.path.join(dataset_dir, 'metadata.csv')
    df_meta = pd.read_csv(metadata_path)
    
    sample_row = df_meta[df_meta['density'] == 'sparse'].iloc[0]
    img_id = sample_row['image_id']
    img_path = os.path.join(dataset_dir, sample_row['split'], 'images', f"{img_id}.png")
    
    print(f"Selected representative image: {img_id} ({img_path})")
    
    # 1. Naive Prompt Execution
    print("\n--- 1. Testing Naive Prompt ---")
    naive_response = query_vlm_with_fallback(img_path, NAIVE_PROMPT)
    print(f"Naive Prompt Response:\n{naive_response}\n")
    with open(os.path.join(OUTPUT_DIR, "naive_prompt_response.txt"), "w") as f:
        f.write(f"Image ID: {img_id}\nPrompt: {NAIVE_PROMPT}\n\nResponse:\n{naive_response or ''}")
        
    # 2. Structured Prompt Framing Execution
    print("\n--- 2. Testing Structured Prompt ---")
    structured_response = query_vlm_with_fallback(img_path, STRUCTURED_PROMPT)
    ...
    for i in range(1, 4):
        print(f"Run {i}...")
        resp = query_vlm_with_fallback(img_path, STRUCTURED_PROMPT, temperature=0.7)
